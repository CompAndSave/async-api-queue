...
class AsyncApiQueue {
  ...
  static async initialize(redisUrl, prefix = "", size = 5, expiration = 60 * 60 * 24) {
    ...
    AsyncApiQueue.prefix = prefix;
    AsyncApiQueue.client = redis.createClient({ url: redisUrl });
    AsyncApiQueue.client.on("error", (error) => console.error(error));
    await AsyncApiQueue.client.connect();
   ...
   */
  static async close() { await AsyncApiQueue.client.close(); }
  ...
  static async setRedis(key, value) {
    return Promise.resolve(await AsyncApiQueue.client.set(AsyncApiQueue.prefix + key, value, { EX: AsyncApiQueue.expiration }));
  }
  ...
  static async getRedis(key) {
    return Promise.resolve(await AsyncApiQueue.client.get(AsyncApiQueue.prefix + key));
  }
  ...
  static async delRedis(key) {
    return Promise.resolve(await AsyncApiQueue.client.del(AsyncApiQueue.prefix + key));
  }
}
